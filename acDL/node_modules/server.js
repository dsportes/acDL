...
AC.Server.prototype.dumpV = function(line){
	this.filtre = null;
	var qs = this.initQs("dumpV");
	qs.l = line;
	this.post(qs, null, 3, line + "line/Versions.json.gz");
};

AC.Server.prototype.dumpC = function(line, col, batch){
	this.setFiltre(null, col);
	var qs = this.initQs("dumpC");
	qs.l = line;
...
AC.Server.prototype.load = function(line, fileName, first){
...
AC.Server.prototype.loadD = function(line, fileName, urlSrc, urlDest){
...
AC.Server.prototype.post = function(qs, fileName, toSave, name){
	...
	if (fileName) {
		try {
			var stats = AC.fs.statSync(fileName);
			size += stats.size;
			var readStream = AC.fs.createReadStream(fileName);
		} catch(e) {
	...
	var resSize = 0;
	if (toSave == 1) {
		self.header = this.dirName + name + "_";
	} else if (toSave == 2) {
		...
		self.stream = AC.fs.createWriteStream(d + "/" + name);
	} else if (toSave == 3){
		var d = this.dirName;
		self.stream = AC.fs.createWriteStream(d + "/" + name);		
	}
				...
				if (!toSave){
					...
					if (!self.stream) {
						for(var i = 0; i < chunk.length; i++){
							...
							if (!c){
								s = i + 1;
								self.stream = AC.fs.createWriteStream(self.header + ".zip");
								break;
							} else
								self.header += String.fromCharCode(c);
						};
					}
